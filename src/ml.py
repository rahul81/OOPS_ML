...
class ML():
    ...
    def make_clf_parmas(self):
        ...
        clf=MultinomialNB()
        params={'alpha':[.3,.4,.5,.6,.7,.8,.9,1,1.5,2]}
    ...
    def grid_search(self, X_train, X_test, y_train,y_test,n_splits):
        ...
        for clf, params in clf_and_params:
            ...
            grid_search_clf = GridSearchCV(clf, params, cv=self.cv, \
                scoring=make_scorer(roc_auc_score))
            grid_search_clf.fit(self.X_train, self.y_train)
            self.Y_pred = grid_search_clf.predict(self.X_test)
            print(f"Traning score on model : {self.current_clf_name} : {grid_search_clf.score(X_train,y_train)}")
            ...
            pickle.dump(self.current_clf_name, open(f"models/{self.current_clf_name}", 'wb'))
    ...
    def model_fit(self,model, X_train,y_train,x_test,y_test):
        ...
        print("================================================")
        print(f"Train roc auc score for model {model.__class__.__name__} : {roc_auc_score(y_train,preds)}")

        preds = model.predict(x_test)
        print(f"Test roc auc score for model {model.__class__.__name__} : {roc_auc_score(y_test,preds)}")
        print("================================================")
    ...
    def model_inference(self,model,features,targets):
        ...
        if targets is not None:
            ...
            print("Test ROC AUC score on model {} : {}".format(model.__class__.__name__,roc_auc_score(targets,preds)))
