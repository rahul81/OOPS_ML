...
class Sentiment_Analysis():
    ...
    def get_splits(self, features,targets,test_size):
        ...
        Xtrain,Xtest,Ytrain,Ytest = train_test_split(features,targets, test_size=test_size,
        stratify=targets, random_state=42)

        return Xtrain,Xtest, Ytrain,  Ytest

    def train_model(self,df,model,sample_data=True,sampling_ratio=(.4,.7),test_size=.2):

        features,targets = self.transform(df)
        Xtrain,Xtest, Ytrain, Ytest= self.get_splits(features,targets,test_size)

        if sample_data:
            Xtrain,Ytrain = self.make_features.sample_data(Xtrain,Ytrain,sampling_ratio[0],sampling_ratio[1])

        model = self.ml.model_fit(model, Xtrain, Ytrain,Xtest,Ytest)

        return model

    def train_gridsearch(self,df,sample_data=True,sampling_ratio=(.4,.7),n_splits=5,test_size=.2):
